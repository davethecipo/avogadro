...
SelectionConsecutiveDistanceExtension::
    ~SelectionConsecutiveDistanceExtension() {}
...
QUndoCommand *
SelectionConsecutiveDistanceExtension::performAction(QAction *action,
                                                     GLWidget *widget) {

  QString fileName =
      outputFilename(m_molecule, widget, QString("text"), QString("txt"),
                     QString("bonds"), "Save distance between atoms to");

  QFile file(fileName);
  file.open(QIODevice::WriteOnly | QIODevice::Text);
  QTextStream outStream(&file);
  outStream << "# Bond number," << '\t' << "distance [Angstrom]," << '\t'
            << "From atom," << '\t' << "To atom" << endl;

  PrimitiveList selectedPrimitives = widget->selectedPrimitives();
  const QList<Primitive *> AtomsList =
      selectedPrimitives.subList(Primitive::AtomType);
  for (int i = 0; i < AtomsList.count() - 1; i++) {
    // p is a pointer to a Primitive; save the memory location of the i-th
    // element in it
    ...
    outStream << i + 1 << '\t' << length << '\t' << selected->index() + 1
              << '\t' << nextAtom->index() + 1 << endl;
  }
...
Q_EXPORT_PLUGIN2(SelectionConsecutiveDistanceExtension,
                 Avogadro::SelectionConsecutiveDistanceExtensionFactory)
